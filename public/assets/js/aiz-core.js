            ...
            setTimeout(function () {
                ...
                if (data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        // ✅ FIX: build safe full url (avoid companyuploads / double slashes)
                        var fileUrl =
                            (AIZ.data.fileBaseUrl || "").replace(/\/+$/, "") +
                            "/" +
                            String(data[i].file_name || "").replace(/^\/+/, "");
        
                        if (data[i].type === "image") {
                            thumb = '<img src="' + fileUrl + '" class="img-fit">';
