...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="align-items-center">
        <h1 class="h3">{{translate('Inquired Customers')}}</h1>
    </div>
</div>
...
<div class="card">
    <form class="" id="sort_customers" action="" method="GET">
        <div class="card-header row gutters-5">
            <div class="col">
                <h5 class="mb-0 h6">{{translate('Customers Who Submitted Inquiries')}}</h5>
            </div>
        ...
        <div class="card-body">
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{translate('Total Inquiries')}}</th>
                    ...
                    @foreach($users as $key => $user)
                        @if ($user != null)
                            <tr>
                                ...
                                <td>{{$user->phone}}</td>
                                <td>
                                    <span class="badge badge-inline badge-info">{{$user->product_queries_count ?? 0}}</span>
                                </td>
                                <td>
                                ...
                                <td>{{single_price($user->balance ?? 0)}}</td>
                                <td>
                                ...
                                <td class="text-right">
                                    @if($user->email_verified_at != null && (auth()->user()->user_type == 'admin' || auth()->user()->can('login_as_customer')))
                                    ...
                                    @endif
                                    @if(auth()->user()->user_type == 'admin' || auth()->user()->can('ban_customer'))
                                        @if($user->banned != 1)
                                        ...
                                        @endif
                                    @endif
                                    @if(auth()->user()->user_type == 'admin' || auth()->user()->can('delete_customer'))
            ...
            <div class="text-center p-4">
                <p class="mb-0">{{translate('No customers have submitted inquiries yet.')}}</p>
            </div>
            @endif
        </div>
    </form>
